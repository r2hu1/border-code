...
function createModelFn(provider: string, apiKey: string, modelName: string) {
  switch (provider) {
    ...
    case "groq":
      return createOpenAI({ baseURL: "https://api.groq.com/openai/v1", apiKey }).chat(modelName);
    case "mistral":
...
export async function chat(options: ChatOptions): Promise<ChatResult | undefined> {
  const {
    messages,
    mode,
    onText,
  ...
  if (abortSignal?.aborted) {
    onError?.({ type: "aborted", message: "Request was aborted.", retryable: false });
    return undefined;
  }
  ...
  try {
    ...
    if (
      ...
      onError?.({ type: "aborted", message: "Request was aborted.", retryable: false, raw: err });
      return undefined;
    }
  ...
  try {
    [reasoningText, steps] = await Promise.all([
      streamResult.reasoning.then((parts) =>
        parts.map((r) => r.text).join("\n") || undefined
      ),
      streamResult.steps,
    ]);
  } catch (err) {
